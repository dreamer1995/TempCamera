...
Texture3D skyCameraVolumeLUT : register(t3);
Texture2D skyMap : register(t4);
SamplerState splr;
...
float4 main(float2 uv : Texcoord) : SV_Target
{
	...
	float3 Color = skyMap.SampleLevel(splr, uv, 0).rgb;
	//float3 Color = skyCameraVolumeLUT.SampleLevel(splr, float3(uv, 31u), 0).rgb;
